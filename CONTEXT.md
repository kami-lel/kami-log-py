...
- `_calc_line_end(args)` — resolves `args.newline` to the `end` string (`""` or `"\n"`): `False` forces `""`; `True` or `None` (the default) both resolve to `"\n"`, since every caller has already stripped stdin's own trailing newline from the content before printing, so auto mode always restores exactly one, matching `-n`; used both as the banner/color handlers' `print()` `end` and to decide the logger's terminator suppression
- `_comment_banner_parser_main(args)` — handler that reads the raw stdin line, derives `content` via `.rstrip("\n")`, maps CLI modes to `_gen_comment_banner_generic`, builds an `AnsiRenderer` honoring `-C`, then prints the returned line to stdout with the newline `_calc_line_end(args)` decides
...
- `_comment_banner_zero_parser_main(args)` — handler that reads raw stdin, splits it into `lines` (one banner line per stdin line), builds an `AnsiRenderer` honoring `-C`, calls `gen_comment_banner_zero`, then prints the returned banner with the newline `_calc_line_end(args)` decides
...
- `_print_colored_stdin_line(args, style)` — shared handler body for `color` and `color-grey`: reads a single stdin line, applies `style` via a plain (non-`-C`-aware) `AnsiRenderer`, and prints with the newline `_calc_line_end(args)` decides
...
- `_logger_parser_main(args)` — handler that resolves `LEVEL` and `--time-format` through `_LOGGER_LEVEL_MAP` / `_LOGGER_TIME_FORMAT_MAP`, calls `getLogger(args.name, datefmt=…)`, applies the verbosity threshold, then logs each stdin line at the resolved level; `_calc_line_end(args) == ""` decides once whether the final stdin line's trailing newline should be suppressed — forced by `-N`/`--no-newline` only, since auto mode and `-n`/`--newline` both keep it — and only then switches every handler's `terminator` to `""` immediately before the final line is logged, so earlier records always keep their line breaks
